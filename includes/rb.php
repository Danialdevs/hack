...
class OODBBean implements \IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	...
	*/
	 ...
	 * includes NULL and FALSE values).
	 *
	 * Usage:
	 *
	 * <code>
...
interface QueryWriter
{
	 ...
	 * code for the writer. If the includes specials flag is set it will also
	 ...
	 * @param array  $columnsPartOfIndex columns to includes in index
	 *
	 * @return void
	 */
...
abstract class AQueryWriter
{
	 ...
	 * includes a source table, prefix the key with 'on_table_<SOURCE>_'.
	 *
...
class Fluid extends Repository
{
	 ...
	 * This may includes adding a new column or widening an existing column to hold a larger
...
class LabelMaker
{
	...
	public function enum( $enum )
	{
		 ...
		 * search type, people can also includes references in the SQL part, so avoiding
...
class Facade
{
	...
	*/
	 ...
	 * @param string|NULL $sql      SQL query to find the desired bean, starting right after WHERE clause
	 * @param array       $bindings array of values to be bound to parameters in query
	 * @param string|NULL $snippet  SQL snippet to includes in query (for example: FOR UPDATE)
	 *
	 ...
	 * at least MariaDB 10.2.2 or Postgres. This method does not includes
